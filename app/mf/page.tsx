...
export default function MFCalculatorPage() {
  ...
  const [currentCorpus, setCurrentCorpus] = useState(50000)
  ...
  const usedMonthly = useMemo(() => {
    ...
    const req = sipRequiredForTarget(targetAmount, currentCorpus, expectedReturn, years)
    return Math.round(req)
  }, [mode, monthlyContribution, targetAmount, expectedReturn, years, currentCorpus])

  const fv = useMemo(() => futureValueMF(currentCorpus, usedMonthly, expectedReturn, years), [currentCorpus, usedMonthly, expectedReturn, years])

  const invested = useMemo(() => currentCorpus + usedMonthly * 12 * Math.max(0, years), [currentCorpus, usedMonthly, years])
  ...
  const projData = useMemo(() => {
    ...
    for (let y = 0; y <= nYears; y++) {
      ...
      const fvL = futureValueLumpSum(currentCorpus, expectedReturn, y)
      const fvS = usedMonthly * af
      const value = fvL + fvS
      const real = showReal ? value / Math.pow(1 + inflation / 100, y) : value
      arr.push({ year: y, fv: value, real })
    }
    return arr
  }, [expectedReturn, years, usedMonthly, currentCorpus, showReal, inflation])
      ...
      <header className="border-b bg-card/50 backdrop-blur-sm">
        <div className="container mx-auto px-4 py-6 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-bold tracking-tight">Mutual Fund Calculator</h1>
              <p className="text-muted-foreground">Combined SIP + existing corpus growth</p>
            </div>
          </div>
          <div className="flex items-center gap-4">
          ...
          <Card className="shadow-sm">
            ...
            <CardContent className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                <Input type="number" value={currentCorpus} min={0} step={1000} onChange={(e) => setCurrentCorpus(Number(e.target.value))} />
              </div>
              <div className="space-y-2">
          ...
          <Card className="shadow-sm">
            ...
            <CardContent>
              ...
              <div className="text-3xl font-bold">
                {formatCurrency(mode === "target" ? usedMonthly : fv)}
              </div>
              {mode === "project" && (
